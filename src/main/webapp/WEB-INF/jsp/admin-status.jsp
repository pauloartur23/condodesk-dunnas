...
<head>
    <meta charset="UTF-8">
    <title>CondoDesk | Status de Chamado</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge { background: rgba(124,58,237,0.15); color: #A78BFA; padding: 4px 10px; border-radius: 8px; font-size: 0.8rem; border: 1px solid rgba(124,58,237,0.3); }
...
<aside class="sidebar">
    ...
    <a href="/admin/status" class="nav-link active">🔄 Status</a>
...
<main class="main">
    ...
    <div class="grid">
        <div class="card">
            <h3>Cadastrar Novo Status</h3>
            <form action="/admin/status/salvar" method="post">
                <label>Nome do Status</label>
                <input type="text" name="nome" placeholder="Ex: AGUARDANDO_PECA" required>
                <button type="submit" class="btn">SALVAR STATUS</button>
            </form>
            <c:if test="${param.sucesso == 'true'}">
                <p class="success">✅ Status cadastrado com sucesso!</p>
            </c:if>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <c:forEach var="s" items="${statusList}">
                        <tr><td><span class="badge">${s.nome}</span></td></tr>
                    </c:forEach>
                </tbody>
            </table>
        </div>
